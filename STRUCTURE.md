...
When bundling a package in Python, non-source files such as licenses, changelogs, and other metadata that should be considered *part* of the package, must be defined within the [`MANIFEST.in`](https://packaging.python.org/guides/using-manifest-in/) file. After generation of your project, this file should have configuration for most everything needed during packaging the project. If new static files that need to be packaged are introduced, you will need to update this file.

### `Pipfile`

Instead of using `requirements.txt` this file declares your development dependencies. This file is built and managed by [Pipenv](https://pipenv.kennethreitz.org/en/latest/) which is *currently* considered by [PyPi](https://pypi.org/) the defacto standard for managing Python dependencies.

Installing, removing, and analyzing dependencies are done through the `pipenv` utility (which is an installed prerequisite for this project template). Pipenv also manages virtual environments for your project, which is later discussed in the “Project Tooling” sections.

### `Pipfile.lock`

When locking dependencies with Pipenv, an accompanying `Pipfile.lock` (JSON) file is generated. If a `Pipfile.lock` file is included, the `pipenv` utility will install its dependencies as declared within the locked dependencies rather than directly from the declared dependencies (in `Pipfile`).

[Pipenv](https://pipenv.kennethreitz.org/en/latest/) will automatically update this lock file when the declared dependencies are updated. You can skip this locking process (as it does dependency-resolution) using the `--skip-lock` flag with your usage of the `pipenv` utility. For example:

```console
$ pipenv install attrs --skip-lock
Installing attrs
...
```
